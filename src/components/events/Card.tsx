...
const Card = ({ event }: Props) => {
  ...
  return (
    <div className={`rounded-2xl overflow-hidden shadow-lg relative h-96 ${styles.card}`}>
      ...
      <div className={`menu-link bg-white absolute right-5 top-5 rounded pb-3 z-50 ${!showMenu && 'hidden'}`}>
        <a href="#" className='font-semibold text-cyan-800 block pr-3 pl-3 pt-3'>Edit Event</a>
        <a href="#" className='font-semibold text-cyan-800 pr-3 pl-3 pb-3'>View Public Event Page</a>
      ...
      <div className='bg-gradient rounded-2xl absolute top-20 bottom-0'>
        ...
        <div className="py-4">
          <div className="font-semibold text-xl mb-2 text-center stone:900"> {event?.series?.name}</div>
          <p className="text-gray-600 text-base text-center">
            {event?.series?.name} | <span className={`font-semibold ${event?.status === 'closed' ? 'text-red-500' : event?.status === 'open' ? 'text-green-500' : 'text-blue-500'}`}>Event {event?.status}</span>
          </p>
        ...
        <div className="flex py-3 place-items-center flex-col font-semibold text-cyan-800">
